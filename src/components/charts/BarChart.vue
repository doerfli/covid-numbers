...
export default class BarChart extends Vue {
  ...
  private paintChart (dataPoints: Array<DataPoint>) {
    ...
    this.$refs.chart.append(Plot.plot({
      marks: [
        Plot.barY(dataPoints, {
          x: "xValue",
          y: "yValue",
        })
      ]
    , width: this.$refs.chart.clientWidth, height: this.$refs.chart.clientHeight}));
    ...
    //   ? (dataPoints.map((e) => Math.min(e.yValue ?? 0, e.y2Value ?? 0, e.y3Value ?? 0)).reduce((a, b) => Math.min(a, b))) * 1.01
    //   : 0;
    // const max = dataPoints.length > 0
    //   ? (dataPoints.map((e) => Math.max(e.yValue ?? 0, e.y2Value ?? 0, e.y3Value ?? 0)).reduce((a, b) => Math.max(a, b))) * 1.01
    //   : 1;
    // // console.log(max);
    //
    // // intialize chart
